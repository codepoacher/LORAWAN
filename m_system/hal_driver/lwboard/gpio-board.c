...
void GpioMcuInit( Gpio_t *obj, PinNames pin, PinModes mode, PinConfigs config, PinTypes type, uint32_t value )
{
    if( pin < IOE_0 )
    {
		...
		//debug("pin:0x%x , mode:0x%x, pull:0x%x, speed:0x%x, alternate: 0x%x\r\n",GPIO_InitStructure.Pin, \
				GPIO_InitStructure.Mode, GPIO_InitStructure.Pull, GPIO_InitStructure.Speed, GPIO_InitStructure.Alternate);
